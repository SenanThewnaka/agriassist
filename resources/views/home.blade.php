        ...
        <div class="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
            <div class="flex-1 text-center lg:text-left space-y-10 reveal origin-bottom">
                ...
                <div
                    ...
                    <span>{{ __('AI Diagnostics Live') }}</span>
                </div>
            ...
            <div class="flex-1 reveal w-full" style="transition-delay: 200ms">
                <div
                    ...
                    <img src="{{ asset('images/hero.png') }}" alt="Sri Lankan Farmer in Rice Paddy"
...
<section class="bg-[#0a1e15] text-white py-24 sm:py-32 relative overflow-hidden" x-data="weatherApp()" x-init="init()">
    <!-- Decorative bg -->
    <div
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <div class="space-y-4">
                ...
                <h2 class="text-5xl md:text-7xl font-black tracking-tighter text-white leading-none">{{
                    __('Precision.') }}</h2>
                <p class="text-emerald-200/90 max-w-2xl text-xl font-medium pt-4">{{ __('Predictive 7-day agricultural
                    modeling to optimize your resource deployment and protect against imminent threats.') }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
            ...
            <div
                ...
                <div class="space-y-6 relative z-10">
                    ...
                    <p class="font-bold text-lg leading-relaxed text-amber-950/80" x-text="verdict">Calibrating
                        sensors for 7-day agricultural recommendation...</p>
                </div>
...
<section id="features" class="max-w-7xl mx-auto px-6 py-24 sm:py-32">
    <div class="text-center mb-16 sm:mb-24 reveal">
        <h2 class="text-5xl md:text-6xl font-black mb-6 tracking-tighter text-emerald-950 dark:text-white">{{ __('Built
            for the Field') }}</h2>
        <p class="text-emerald-700/80 dark:text-emerald-400/80 max-w-2xl mx-auto text-xl font-bold">{{ __('Resilient,
            extreme precision algorithms wrapped in an interface anyone can use.') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12">
        <div class="group p-8 sm:p-10 bg-white dark:bg-[#081811] rounded-[2.5rem] border-4 border-emerald-100 dark:border-emerald-900 shadow-xl hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors reveal"
            ...
            <h3 class="text-3xl font-black tracking-tight text-emerald-950 dark:text-white mb-4">{{ __('Neural Speed')
                }}</h3>
            <p class="text-emerald-800/80 dark:text-emerald-300/80 text-lg font-medium leading-relaxed">{{ __('Deep
                learning models deliver field diagnostics in under 1.5 seconds, specifically optimized for varied
        ...
        <div class="group p-8 sm:p-10 bg-white dark:bg-[#081811] rounded-[2.5rem] border-4 border-emerald-100 dark:border-emerald-900 shadow-xl hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors reveal"
            ...
            <h3 class="text-3xl font-black tracking-tight text-emerald-950 dark:text-white mb-4">{{ __('Verified
                Prescriptions') }}</h3>
            <p class="text-emerald-800/80 dark:text-emerald-300/80 text-lg font-medium leading-relaxed">{{ __('Access
                expert-verified treatment paths tailored perfectly for Sri Lankan cash crops and native agricultural
                environments.') }}</p>
        </div>
        ...
        <div class="group p-8 sm:p-10 bg-white dark:bg-[#081811] rounded-[2.5rem] border-4 border-emerald-100 dark:border-emerald-900 shadow-xl hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors reveal"
            ...
            <p class="text-emerald-800/80 dark:text-emerald-300/80 text-lg font-medium leading-relaxed">{{ __('Core
                diagnostic models queue up scans when offline, instantly processing the moment network connection
                returns.') }}</p>
        </div>
    </div>
</section>
...
<script>
    function weatherApp() {
        return {
            ...
            insights: [],
            forecast: [],
            verdict: 'Acquiring atmospheric data...',
            criticalAlert: null,
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        err => {
                            ...
                            this.locationName = "Colombo, SL (Default)";
                        }
                    );
                } else {
                    this.fetchWeather(6.9271, 79.8612);
                    this.locationName = "GPS Not Available";
                }
            },
            ...
            async fetchWeather(lat, lon) {
                try {
                    ...
                    this.locationName = this.locationName || `Coordinates: ${lat.toFixed(2)}, ${lon.toFixed(2)}`;

                    // Process daily forecast
                    this.processForecast(data.daily);

                } catch (e) {
                    this.verdict = "Telemetry offline. Please verify connectivity.";
            ...
            processForecast(daily) {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    let action = { type: 'neutral', text: 'Routine Maintenance', icon: 'clipboard-list' };

                    if (windMax < 15 && rainProb < 20) {
                        action = { type: 'spray', text: 'Optimal Spraying', icon: 'spray-can' };
                    }
                    else if (rainSum >= 2 && rainSum <= 10 && (i === 0 || i === 1)) {
                        action = { type: 'fertilize', text: 'Apply Fertilizer', icon: 'leafy-green' };
                    }
                    else if (rainProb > 70 || windMax > 30) {
                        action = { type: 'danger', text: 'Secure Equipment', icon: 'shield-alert' };
                    }
                    else if (tempMax > 33) {
                        action = { type: 'danger', text: 'Heat Stress Risk', icon: 'sun-dim' };
                        extremeHeatCount++;
                    }
            ...
            generateStrategicVerdict(daily, heatCount) {
                ...
                if (current.wind < 15 && current.rain < 20) {
                    this.insights.push({ text: 'Spray Window Active', icon: 'spray-can', type: 'good' });
                }
                if (current.humidity > 80 && current.temp > 25) {
                    this.insights.push({ text: 'Fungal Risk Elevated', icon: 'biohazard', type: 'danger' });
                }
                ...
                if (heatCount >= 3) {
                    this.criticalAlert = {
                        title: "Drought & Heat Stress Imminent",
                        message: "Multiple days in the 7-day forecast exceed safe temperature limits. Prepare heavy irrigation scaling and deploy shade nets over nurseries."
                    };
                    this.verdict = "Severe thermal stress approaching. Ensure water reserves are full and avoid transplanting seedlings.";
                } else if (avgRain > 70) {
                    this.criticalAlert = {
                        title: "Flood/Washout Protocol",
                        message: "High precipitation volume expected over the next 72 hours. Suspend all chemical spraying and clear field drainage routes immediately."
                    };
                    this.verdict = "Heavy monsoon conditions predicted. Protect exposed inputs and secure loose infrastructure.";
                } else if (current.humidity > 85 && current.temp > 28) {
                    this.verdict = "Extreme fungal breeding conditions right now. Scout all perimeters for exact disease footprints and run a rapid diagnosis scan.";
                } else {
                    this.insights.push({ text: 'Conditions Optimal', icon: 'check-circle-2', type: 'good' });
                    this.verdict = "Stable metrics. Excellent 7-day conditions for executing precision agriculture protocols. Refer to the action planner below.";
                }
            }
        }
    }
